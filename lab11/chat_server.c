...
int read_from(int client_index, struct sockname *usernames) {
    ...
    // create reply
    strncpy(rep, username, strlen(username) + 1);
    strncat(rep, ": ", BUF_SIZE - strlen(rep));
    if(strlen(username) + 2 > BUF_SIZE) {
        rep[BUF_SIZE] = '\0';
    } else {
        rep[strlen(usernames[client_index].username) + 2] = '\0';
    }
    strncat(rep, buf, 2*BUF_SIZE + 3 - strlen(rep));
    rep[2*BUF_SIZE + 2] = '\0';
    ...
    for(int i = 0; i < MAX_CONNECTIONS; i++) {
        if(usernames[i].sock_fd != -1) {
            if(write(usernames[i].sock_fd, rep, strlen(rep)) != strlen(rep)) {
